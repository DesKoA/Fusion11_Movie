...
public class MyCalendar extends JInternalFrame { // CalendarDataManager의 GUI + 메모기능 + 시계
...
//	}
	...
	public MyCalendar() { // 구성요소 순으로 정렬되어 있음. 각 판넬 사이에 빈줄로 구별
		setToday();
...
//		selectedDate.setBorder(BorderFactory.createEmptyBorder(0, 5, 0, 0));
		...
		pack();
		setVisible(true);
		// BorderLayout layout = (BorderLayout)frameSubPaneJDesktopPanelWest.getLayout();
...
//		frameSubPanelWest.setPreferredSize(frameSubPanelWestSize);
		...
		for (int i = 0; i < CAL_HEIGHT; i++) {
			for (int j = 0; j < CAL_WIDTH; j++) {
				...
				if (j == 0)
					...
					fontColor = "blue";
				// dateButs[i][j].setText("<html><font size='4' color=" + fontColor + ">" + calDates[i][j] + "</font></html>");
				dateButs[i][j].setText("<html><font style='font-family: 고딕' size='5' color=" + fontColor + ">" + calDates[i][j] + "</font></html>");
				...
				if (calMonth == today.get(Calendar.MONTH) && calYear == today.get(Calendar.YEAR)
					...
					if (dateButs[i][j].isEnabled()) {
						...
						dateButs[i][j].setBorder(new LineBorder(Color.BLUE, 3));
						dateButs[i][j].setToolTipText("Today");
					}  else if (!dateButs[i][j].isEnabled() && tbb) {
						resetButsTrue();
	...
	// DB와 비교해서
	public void showCalAfterDB(String movTitle) {
		...
		for (int i = 0; i < CAL_HEIGHT; i++) {
			for (int j = 0; j < CAL_WIDTH; j++) {
				...
				String date = calYear + "-" + (calMonth + 1) + "-" + calDates[i][j];
				dateButs[i][j].setEnabled(false);
				dateButs[i][j].setBorderPainted(false);
				for (int l = 0; l < thList.size(); l++) {
					Theaters th = thList.get(l);
					SimpleDateFormat sf = new SimpleDateFormat("yyyy-M-d");
					String thDate = sf.format((th.getMovieDate()));
					int todayInt = today.get(Calendar.DATE);
					if (date.equals(thDate) && todayInt <= calDates[i][j]) {  
						dateButs[i][j].setEnabled(true);
						// dateButs[i][j].setText("<html><font size='4'><strong>" + calDates[i][j] + "</strong></font></html>");
						dateButs[i][j].setText("<html><font style='font-family: 고딕' size='5' color=" + fontColor + "><b>" + calDates[i][j] + "</b></font></html>");
						break;
					}
				}
			}
	...
	private boolean checkMonth(int mon, String movTitle) {
		int month = mon + 1;
		boolean b = false;
		SimpleDateFormat monthSf = new SimpleDateFormat("M");
		TheatersDBManager thMgr = new TheatersDBManager();
		ArrayList<Theaters> thList = thMgr.selectOneMovie(movTitle);
		for (int i = 0; i < thList.size(); i++) {
			Theaters th = thList.get(i);
			String thDate = monthSf.format(th.getMovieDate());
			if (String.valueOf(month).equals(thDate)) {
				b = true;
				break;
			}
		}
	...
	private boolean checkYear(int year, String movTitle) {
		boolean b = false;
		SimpleDateFormat monthSf = new SimpleDateFormat("YYYY");
		TheatersDBManager thMgr = new TheatersDBManager();
		ArrayList<Theaters> thList = thMgr.selectOneMovie(movTitle);
		for (int i = 0; i < thList.size(); i++) {
			Theaters th = thList.get(i);
			String thDate = monthSf.format(th.getMovieDate());
			if (String.valueOf(year).equals(thDate)) {
				b = true;
				break;
			}
		}
	...
	private class ListenForCalOpButtons implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == lYearBut) {
				if (mov.movTitle != null) resetButs();
				moveMonth(-12);
				showCal();
				boolean b = checkYear(calYear, mov.movTitle);
				if (b) mov.calFrame.showCalAfterDB(mov.movTitle);
				tbb = false;
				// showCalAfterDB();
			} else if (e.getSource() == lMonBut) {
				if (mov.movTitle != null) resetButs();
				moveMonth(-1);
				boolean b = checkMonth(calMonth, mov.movTitle);
				if (b) mov.calFrame.showCalAfterDB(mov.movTitle);
				tbb = false;
				// else resetButsTrue();
			...
			for (int i = 0; i < CAL_HEIGHT; i++) {
				for (int j = 0; j < CAL_WIDTH; j++) {
					if (e.getSource() == dateButs[i][j]) {
						...
						mov.movieInfoScreenName.setText("-");
						mov.movieInfoMoneyName.setText("-");
						mov.movieInfoPeopleName.setText("-");
						mov.movieInfoTimeName.setText("-");
						mov.bgAdult.clearSelection();
						mov.bgStudent.clearSelection();
						mov.timePanel.removeAll();
